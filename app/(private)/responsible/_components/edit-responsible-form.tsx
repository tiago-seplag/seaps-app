...
const formSchema = z.object({
  name: z
    .string({ message: "Insira o nome do Responsável" })
    .min(1, { message: "Insira o nome do Responsável" }),
  role: z.string(),
  email: z.string().optional(),
  phone: z.string().optional(),
  organization_id: z.string({ message: "Selecione o Orgão" }),
});

export function EditResponsibleForm({ person }: { person: Person }) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    return axios
      .put("/api/responsible/" + person.id, values)
      .then(() => router.back())
      .catch((e) => {
        if (e.response.data.messages?.length > 0) {
          return e.response.data.messages.map((msg: string) =>
            toast.error(msg),
          );
          ...
          render={({ field }) => (
            <FormItem className="w-full">
              ...
              <Select
                ...
                defaultValue={person.organization_id}
                disabled
              >
                <FormControl>
                  <SelectTrigger>
